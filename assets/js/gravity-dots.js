...
(function () {
  ...
  // The plugin prints `window.GravityDots = { ...settings... }` before
  // this script. Defaults here MUST match includes/defaults.php so the
  // engine still runs if the config is ever missing.
  var IN = (window.GravityDots && typeof window.GravityDots === "object") ? window.GravityDots : {};
  ...
  /* ---------- 2 · Early exits ---------- */
  var canvas = document.getElementById("gravity-dots");
  if (!canvas) return; // the plugin injects <canvas id="gravity-dots"> in wp_footer
